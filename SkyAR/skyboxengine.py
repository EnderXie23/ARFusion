...
class SkyBox():
    ...
    def skymask_refinement(self, G_pred, img):
        ...
        # guide = torch.from_numpy(img[:, :, 2]).unsqueeze(0).unsqueeze(0).float().cuda()  # (1,1,H,W)
        # src = torch.from_numpy(G_pred[:, :, 0]).unsqueeze(0).unsqueeze(0).float().cuda()  # (1,1,H,W)
        ...
        # refined_skymask = GuidedFilter(r, eps)(guide, src).cpu()
        # refined_skymask = refined_skymask.squeeze(0).squeeze(0).cpu().numpy()
        refined_skymask = guidedFilter(img[:,:,2], G_pred[:,:,0], r, eps)
        # print("Size of refined_skymask:", refined_skymask.size())
